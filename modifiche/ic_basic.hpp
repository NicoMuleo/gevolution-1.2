	...
	else
{
	...
	if(dist>=r2){
		...
		if (dist>=r1o){
			...
			cor= ((4.5*Q+3.*param[1]*param[1]*r2*r2)/(7.5*Q))*((r1*r1*r1/(dist*dist))*(0.25*omega_k*(r1*r1- r1*r1*5./3.) - (5./4.)*Q*delta_1*delta_1)+ (5.*r2*r2*r2/(dist*dist))*(0.25*Q*(pow(delta_1,3)-pow(delta_d,3))))- (2.*r2/(3.*Q))*(1.5*Q*delta_d- 3.*0.5*Q*(Q+L-1.)*delta_d*delta_d);
			inside = 0.;
		}
		else{
...
//TODO Add here the initial potential field nicmodify
	...
	if(dist>= r2) {
		...
		if(dist>= r1){ 
			corR= - 0.75* Q*delta_d * delta_d + 0.5* Q*(Q+ param[5]-1.)*delta_d * delta_d * delta_d ;  
			//corR= - Q* (0.5* dist *dist + r2*r2*r2/dist - 1.5 *r2*r2);
			inside=0.;
		}
		else{
			corR= omega_k*(dist*dist - r1*r1)*0.25-  0.75 *Q * delta_1*delta_1 + 0.5* Q*(Q+ param[5]-1.)*delta_1 * delta_1 * delta_1; 
